...
- Added a **"Hold LEDs on Black Screen"** setting (mobile and TV): a fully black screen is shown for a configurable number of seconds before the LEDs hold the last non-black frame, so the lights no longer drop to the default colour when a video is paused (0 = hold instantly). The value is shown with its `seconds` unit in both UIs.
